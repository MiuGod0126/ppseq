...
# from ppseq.modules.multihead_attention import MultiheadAttention
# if TYPE_CHECKING:
#     from ppseq.modules.multihead_attention import MultiheadAttention
#
# try:
...
# try:
#     import paddle_xla.core.xla_model as xm
# except ImportError:
#     xm = None
...
#     from ppseq import checkpoint_utils
#
#     deprecation_warning(
...
#     device = device or paddle.cuda.current_device()
#
#     def _move_to_cuda(tensor):
#         # non_blocking is ignored if tensor is not pinned, so we can always set
#         # to True (see github.com/PyTorchLightning/pypaddle-lightning/issues/620)
...
#             tensor = tensor.to(dtype=paddle.float32)
#         return tensor.cpu()
#
...
def parse_embedding(embed_path):
    ...
    with open(embed_path) as f_embed:
        ...
        for line in f_embed:
            ...
            embed_dict[pieces[0]] = paddle.Tensor(
...
def make_positions(tensor, padding_idx: int, onnx_trace: bool = False):
    ...
    return (paddle.cumsum(mask, axis=1).type_as(mask) * mask).long() + padding_idx
...
def buffered_arange(max):
    if not hasattr(buffered_arange, "buf"):
        buffered_arange.buf = paddle.LongTensor()
    if max > buffered_arange.buf.numel():
        buffered_arange.buf.resize_(max)
        paddle.arange(max, out=buffered_arange.buf)
...
def convert_padding_direction(
    ...
    buffered = paddle.empty(0).long()
    if max_len > 0:
        paddle.arange(max_len, out=buffered)
    range = buffered.type_as(src_tokens).expand_as(src_tokens)
    num_pads = pad_mask.long().sum(dim=1, keepdim=True)
    if right_to_left:
        index = paddle.remainder(range - num_pads, max_len)
    else:
        index = paddle.remainder(range + num_pads, max_len)
...
def item(tensor):
    ...
    # if paddle.is_tensor(tensor) and tensor.device.type == "xla":  # no tpu
...
def multi_tensor_total_norm(grads, chunk_size=2048 * 32) -> paddle.Tensor:
    ...
    for device in per_device_grads.keys():
        ...
        if device.type == "cuda":
            ...
            has_inf = paddle.zeros((1, 1), dtype=paddle.int, device=device)
            with paddle.cuda.device(device):
                norm = multi_tensor_l2norm(
                    chunk_size, has_inf, [cur_device_grads], False
                )
            norms.append(norm[0].to(paddle.cuda.current_device()))
        else:
            norms += [paddle.norm(g, p=2, dtype=paddle.float32) for g in cur_device_grads]
    total_norm = paddle.norm(paddle.stack(norms))
    return total_norm
...
def clip_grad_norm_(params, max_norm, aggregate_norm_fn=None) -> paddle.Tensor:
    ...
    if len(grads) == 1:
        total_norm = paddle.norm(grads[0], p=2, dtype=paddle.float32)
    else:
        if multi_tensor_l2norm_available:
            ...
            # norms=paddle.stack(
            #         [paddle.norm(g, p=2, dtype=paddle.float32).to(device) for g in grads]
            #     )
...
def import_user_module(args):
    ...
    if module_path is not None:
        ...
        if not os.path.exists(module_path) and not os.path.isfile(
            ...
            paddleseq_rel_path = os.path.join(os.path.dirname(__file__), args.user_dir)
            if os.path.exists(paddleseq_rel_path):
                module_path = paddleseq_rel_path
            else:
                paddleseq_rel_path = os.path.join(
        ...
        if module_path not in import_user_module.memo:
            ...
            if module_name not in sys.modules:
                ...
                if os.path.exists(tasks_path):
                    from ppseq.tasks import import_tasks

                    import_tasks(tasks_path, f"{module_name}.tasks")

                models_path = os.path.join(module_path, "models")
                if os.path.exists(models_path):
                    from ppseq.models import import_models
...
def log_softmax(x, dim: int, onnx_trace: bool = False):
    if onnx_trace:
        ...
        return F.log_softmax(x, dim=dim, dtype=paddle.float32)
...
def get_rng_state():
    state = {"paddle_rng_state": paddle.get_rng_state()}
    if xm is not None:
        state["xla_rng_state"] = xm.get_rng_state()
    if paddle.cuda.is_available():
        state["cuda_rng_state"] = paddle.cuda.get_rng_state()
    return state


def set_rng_state(state):
    paddle.set_rng_state(state["paddle_rng_state"])
    if xm is not None:
        xm.set_rng_state(state["xla_rng_state"])
    if paddle.cuda.is_available():
        paddle.cuda.set_rng_state(state["cuda_rng_state"])


class set_paddle_seed(object):
...
def parse_alignment(line):
    ...
    Returns:
        paddle.IntTensor: packed alignments of shape (2 * m).
    """
    alignments = line.strip().split()
    parsed_alignment = paddle.IntTensor(2 * len(alignments))
...
def new_arange(x, *size):
    ...
    if len(size) == 0:
        size = x.size()
    return paddle.arange(size[-1], device=x.device).expand(*size).contiguous()



class CudaEnvironment(object):
    def __init__(self):
        cur_device = paddle.cuda.current_device()
        prop = paddle.cuda.get_device_properties("cuda:{}".format(cur_device))
        self.name = prop.name
