...
from ppseq.data.indexed_dataset import get_available_dataset_impl
from ppseq.dataclass.configs import (
    CheckpointConfig,
    CommonConfig,
    CommonEvalConfig,
    DatasetConfig,
...
from ppseq.utils import csv_str_list, eval_bool, eval_str_dict, eval_str_list, import_user_module  # noqa
...
def parse_args_and_arch(
    ...
    from ppseq.models import ARCH_MODEL_REGISTRY, ARCH_CONFIG_REGISTRY, MODEL_REGISTRY
    ...
    if hasattr(args, "task"):
        from ppseq.tasks import TASK_REGISTRY

        TASK_REGISTRY[args.task].add_args(parser)
    # if getattr(args, "use_bmuf", False):
    #     # hack to support extra args for block distributed data parallelism
    #     from paddleseq.optim.bmuf import FairseqBMUF

        # FairseqBMUF.add_args(parser)

    # Add *-specific args to parser.
    from ppseq.registry import REGISTRIES
    ...
    if getattr(args, "memory_efficient_fp16", False): # 换fp16为amp
        args.fp16 = True
...
def get_parser(desc, default_task="translation"):
    ...
    # Task definitions can be found under paddleseq/tasks/
    from ppseq.tasks import TASK_REGISTRY
    parser.add_argument(
        "--task",
        metavar="TASK",
...
def add_distributed_training_args(parser, default_world_size=None):
    ...
    if default_world_size is None:
        default_world_size = max(1, paddle.distributed.get_world_size())
...
def add_model_args(parser):
    ...
    # Model definitions can be found under paddleseq/models/
    #
    ...
    from ppseq.models import ARCH_MODEL_REGISTRY
