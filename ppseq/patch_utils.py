...
@patch_to(paddle.Tensor)
def new_zeros(self, size: Union[Tuple[int], List[int]], dtype=None):
    return paddle.zeros(shape=size, dtype=dtype if dtype is None else self.dtype)
...
scatter_torch = patch_to(paddle.Tensor)(scatter_torch)  # 这是同步修改tensor和paddle吗
