...
from ppseq.dataclass.utils import merge_with_parent
from hydra.core.config_store import ConfigStore
from omegaconf import open_dict, OmegaConf

from .ppseq_encoder import PaddleseqEncoder
from .ppseq_decoder import PaddleseqDecoder
from .ppseq_incremental_decoder import PaddleseqIncrementalDecoder
from .ppseq_model import (
    BasePaddleseqModel,
    PaddleseqEncoderDecoderModel,
    PaddleseqEncoderModel,
    PaddleseqLanguageModel,
)
...
def build_model(cfg: PaddleseqDataclass, task):
...
def register_model(name, dataclass=None):
    ...
    def register_model_cls(cls):
        ...
        if not issubclass(cls, BasePaddleseqModel):
            raise ValueError(
                "Model ({}: {}) must extend BasePaddleseqModel".format(name, cls.__name__)
            )
        MODEL_REGISTRY[name] = cls
        if dataclass is not None and not issubclass(dataclass, PaddleseqDataclass):
            raise ValueError(
                "Dataclass {} must extend PaddleseqDataclass".format(dataclass)
            )
        ...
        if dataclass is not None:
            ...
            node = dataclass()
            node._name = name
            cs.store(name=name, group="model", node=node, provider="paddleseq")
...
import_models(models_dir, "ppseq.models")

print(ARCH_MODEL_INV_REGISTRY)