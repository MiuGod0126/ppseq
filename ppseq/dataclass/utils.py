...
def gen_parser_from_dataclass(
    ...
    dataclass_instance: PaddleseqDataclass,
    ...
    def get_kwargs_from_dc(
        dataclass_instance: PaddleseqDataclass, k: str
    ) -> Dict[str, Any]:
    ...
    for k in dataclass_instance._get_all_attributes():
        ...
        if field_name is None:
            continue
        elif inspect.isclass(field_type) and issubclass(field_type, PaddleseqDataclass):
...
def _override_attr(
    sub_node: str, data_class: Type[PaddleseqDataclass], args: Namespace
) -> List[str]:
    overrides = []

    if not inspect.isclass(data_class) or not issubclass(data_class, PaddleseqDataclass):
        return overrides
    ...
    for k, v in data_class.__dataclass_fields__.items():
        ...
        elif isinstance(val, PaddleseqDataclass):
...
def override_module_args(args: Namespace) -> Tuple[List[str], List[str]]:
    ...
    for k in PaddleseqConfig.__dataclass_fields__.keys():
        overrides.extend(
            _override_attr(k, PaddleseqConfig.__dataclass_fields__[k].type, args)
        )
...
def merge_with_parent(dc: PaddleseqDataclass, cfg: DictConfig, remove_missing=True):
    if remove_missing:
