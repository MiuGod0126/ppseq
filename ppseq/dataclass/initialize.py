...
def hydra_init(cfg_name="config") -> None:
    ...
    cs.store(name=f"{cfg_name}", node=PaddleseqConfig)

    for k in PaddleseqConfig.__dataclass_fields__:
        v = PaddleseqConfig.__dataclass_fields__[k].default
        try:
...
def add_defaults(cfg: DictConfig) -> None:
    ...
    from ppseq.tasks import TASK_DATACLASS_REGISTRY
    from ppseq.models import ARCH_MODEL_NAME_REGISTRY, MODEL_DATACLASS_REGISTRY
    from ppseq.dataclass.utils import merge_with_parent
    from typing import Any

    OmegaConf.set_struct(cfg, False)

    for k, v in PaddleseqConfig.__dataclass_fields__.items():
        field_cfg = cfg.get(k)
