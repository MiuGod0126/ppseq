...
encoders
diff：
1.tensor.uniform_ : paddle.uniform
2.torch.IntTensor -> paddle.Tensor
3.torch.IntTensor(n) -> paddle.zeros(shape=[n],dtype="int32")

same:
    paddle.is_tensor,  tensor.dim()

# 外部文件先注释掉无用的，待到需要了再修改
dependency:
    data_utils.numpy_seed √

    encoders.build_tokenizer(args)
    encoders.build_bpe(args)
...
# from ppseq.data import Dictionary, PaddleseqDataset, data_utils, encoders, iterators
from ppseq.data import Dictionary, PaddleseqDataset, data_utils, iterators
...
class PaddleseqTask(object):
    ...
    def load_dataset(
        ...
        split: str,
        combine: bool = False,
        task_cfg: PaddleseqDataset = None,
        **kwargs,
    ):
        ...
        Args:
            ...
            task_cfg (PaddleseqDataset): optional task configuration stored in the checkpoint that can be used
                                         to load datasets
        """
    ...
    def dataset(self, split):
        ...
        """
        from ppseq.data import PaddleseqDataset

        if split not in self.datasets:
            raise KeyError("Dataset not loaded: " + split)
        if not isinstance(self.datasets[split], PaddleseqDataset):
    ...
    def get_batch_iterator(
        ...
        assert isinstance(dataset, PaddleseqDataset)
    ...
    def build_model(self, cfg: PaddleseqDataclass, from_checkpoint=False):
        """
...
class LegacyFairseqTask(PaddleseqTask):
