...
from ppseq.modules.initializer import xavier_uniform_,xavier_normal_,constant_
# try:
...
from ppseq.modules.linear import Linear
from ppseq.modules.ppseq_dropout import PaddleseqDropout
from ppseq.modules.quant_noise import quant_noise
from ppseq.models.ppseq_incremental_decoder import PaddleseqIncrementalDecoder

# # TODO: move this into xformers?
# # TODO: uint8 input type should just output a bool
# def _mask_for_xformers(mask: Tensor, to_dtype: Optional[paddle.dtype] = None):
#     """
#     call to pypaddle multihead accepts three mask types:
...
#     """
#     float_types = [paddle.float, paddle.float16]
...
#         new_mask = paddle.zeros_like(mask, dtype=to_dtype)
...
class MultiheadAttention(PaddleseqIncrementalDecoder):
    ...
    def __init__(
        ...
        self.k_proj = quant_noise(
            Linear(self.kdim, embed_dim, bias=bias), q_noise, qn_block_size
        )
        ...
        if add_bias_kv: # False
            self.bias_k = self.create_parameter(shape=[1, 1, embed_dim],is_bias=True)
            self.bias_v = self.create_parameter(shape=[1, 1, embed_dim],is_bias=True)
        else:
    ...
    #
    #     new_q_weight = paddle.cat(new_q_weight).detach()
    #     new_k_weight = paddle.cat(new_k_weight).detach()
    #     new_v_weight = paddle.cat(new_v_weight).detach()
    #     new_out_proj_weight = paddle.cat(new_out_proj_weight, dim=-1).detach()
    #     new_q_weight.requires_grad = True
    #     new_k_weight.requires_grad = True
    #     new_v_weight.requires_grad = True
    #     new_out_proj_weight.requires_grad = True
    #
    #     new_q_bias = paddle.cat(new_q_bias).detach()
    #     new_q_bias.requires_grad = True
    #
    #     new_k_bias = paddle.cat(new_k_bias).detach()
    #     new_k_bias.requires_grad = True
    #
    #     new_v_bias = paddle.cat(new_v_bias).detach()
    #     new_v_bias.requires_grad = True
    #
    #     self.q_proj.weight = paddle.nn.Parameter(new_q_weight)
    #     self.q_proj.bias = paddle.nn.Parameter(new_q_bias)
    #
    #     self.k_proj.weight = paddle.nn.Parameter(new_k_weight)
    #     self.k_proj.bias = paddle.nn.Parameter(new_k_bias)
    #
    #     self.v_proj.weight = paddle.nn.Parameter(new_v_weight)
    #     self.v_proj.bias = paddle.nn.Parameter(new_v_bias)
    #
    #     self.out_proj.weight = paddle.nn.Parameter(new_out_proj_weight)
    #
    ...
    def _pad_masks(
        ...
        ''' use in : _add_bias, _append_zero_attn'''
        if attn_mask is not None:
            shape = attn_mask.size()[:-1] + paddle.Size([1])
            attn_mask = paddle.cat([attn_mask, attn_mask.new_zeros(shape)], dim=-1)
        if key_padding_mask is not None:
            shape = key_padding_mask.size()[:-1] + paddle.Size([1])
            key_padding_mask = paddle.cat(
                [
                    key_padding_mask,
    ...
    def _add_bias(
        ...
        k = paddle.cat([k, self.bias_k.repeat(1, bsz, 1)])
        v = paddle.cat([v, self.bias_v.repeat(1, bsz, 1)])
    ...
    def _append_zero_attn(
        ...
        zero_attn_shape = k.size()[:-2] + paddle.Size([1]) + k.size()[-1:]
        k = paddle.cat(
            [k, paddle.zeros(zero_attn_shape, dtype=k.dtype, device=k.device)], dim=-2
        )
        v = paddle.cat(
            [v, paddle.zeros(zero_attn_shape, dtype=v.dtype, device=v.device)], dim=-2
        )
        key_padding_mask, attn_mask = self._pad_masks(
            key_padding_mask=key_padding_mask, attn_mask=attn_mask
        )
    ...
    #
    #     if key_padding_mask is not None:
    #         key_padding_mask = _mask_for_xformers(key_padding_mask, to_dtype=paddle.bool)
    ...
    def forward(
        ...
        if (
            ...
            # and not paddle.jit.is_scripting()
            # The Multihead attention implemented in pypaddle forces strong dimension check
            # for input embedding dimention and K,Q,V projection dimension.
            # Since pruning will break the dimension check and it is not easy to modify the pypaddle API,
            # it is preferred to bypass the pypaddle MHA when we need to skip embed_dim_check
            ...
            return F.multi_head_attention_forward(
                ...
                self.num_heads,
                paddle.empty([0]),
                paddle.cat((self.q_proj.bias, self.k_proj.bias, self.v_proj.bias)),
                self.bias_k,
                self.bias_v,
                self.add_zero_attn,
        ...
        if saved_state is not None:
            ...
            if "prev_value" in saved_state:
                ...
                if static_kv:
                    ...
                    v = paddle.cat([prev_value, v], dim=1)
    ...
    def _append_prev_key_padding_mask(
        ...
        if prev_key_padding_mask is not None and static_kv:
            ...
            new_key_padding_mask = paddle.cat(
    ...
    def upgrade_state_dict_named(self, state_dict, name):
        ...
        for k in state_dict.keys():
            if k.endswith(prefix + "in_proj_weight"):
                ...
                items_to_add[prefix + "k_proj.weight"] = state_dict[k][dim : 2 * dim]   # shape可能要颠倒下
