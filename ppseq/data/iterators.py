...
import numpy as np
import paddle
from ppseq.data import data_utils # numpy_seed
...
class StreamingEpochBatchIterator(EpochBatchIterating):
    """A steaming-style iterator over a :class:`paddle.utils.data.IterableDataset`.

    Args:
        dataset (~paddle.utils.data.Dataset): dataset from which to load the data
        ...
        buffer_size (int, optional): the number of batches to keep ready in the
            ...
            default paddle.utils.data.DataLoader preloading is used.
    ...
    def __init__(
        ...
        assert isinstance(dataset, paddle.io.IterableDataset)
...
class EpochBatchIterator(EpochBatchIterating):
    """A multi-epoch iterator over a :class:`paddle.utils.data.Dataset`.

    Compared to :class:`paddle.utils.data.DataLoader`, this iterator:
    ...
    Args:
        dataset (~paddle.utils.data.Dataset): dataset from which to load the data
        collate_fn (callable): merges a list of samples to form a mini-batch
        batch_sampler (~paddle.utils.data.Sampler or a callable): an iterator over batches of
            indices, or a callable to create such an iterator (~paddle.utils.data.Sampler).
...
class BackgroundConsumer(Thread):
    ...
    def run(self):
        ...
        if self.cuda_device is not None:
            paddle.set_device(self.cuda_device)
...
class BufferedIterator(object):
    ...
    def _create_consumer(self):
        self._consumer = BackgroundConsumer(
            ...
            self._iterable,
            self.total,
            # paddle.cuda.current_device() if paddle.cuda.is_available() else None,
        )
